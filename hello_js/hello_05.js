...
// 배열과 반복문
// 동기방식으로 사용되는 일반적인 코드
//      for 시작 / 배열요소들이 출력 / for 종료 가 출력된다.
console.log("for 시작");
...
/**
 * 내부적으로 forEach()가 실행되면서 내부의 함수에서 배열요소를 기준으로 연산이 이루어지고 그 결과를 통째로 다른 변수에 담을 수 있다.
 *
 * IF 배열3의 요소가 5개이다
 *  1. map() 함수 내의 화살표 함수는 5번 실행이 될 것이다
 *  2. 화살표 함수의 코드에서 마지막에 return이 있으므로 return문이 5번 실행될 것이다.
 *  3. 또다른배열.push( return된 결과 )의 코드가 5번 반복된다.
 *
 *  이 결과는 배열3에 담긴 모든 요소에 끝에 문자열 A가 부착되고
 *      문자열 A가 부착된 요소들이 또다른배열에 push되어 새로운 배열이 만들어진다.
 */
...
let 숫자배열 = [2, 3, 4, 5, 6, 1, 2, 3, 5, 7, 8, 9];
console.log("숫자배열", 숫자배열);

// map은 내부의 함수에 요소와 배열의 index값을 매개변수로 전달 할 수 있고
...
// 제곱배열 = 제곱한배열
let 제곱배열 = 숫자배열.map((숫자) => {
  return 숫자 * 숫자;
});
let 제곱한배열 = 숫자배열.map((숫자) => 숫자 * 숫자);
console.log("제곱배열", 제곱배열);
console.log("제곱한배열", 제곱한배열);

// 배열의 요소, index, 자기자신의 복제배열을 매개변수로 전달하여 다양한 코드에서 활용할 수 있다.
제곱한배열.map((요소, index, 원래배열) => {
  console.log(원래배열, "의", index, "번째 요소");
});

// 원 배열에서 특정한 조건에 맞는 요소만 추출하여
// 다른 배열로 만들고 싶을 때
// 내부 코드에서 특정 요소가 연산될 때
// 마지막에 return true인 경우만 해당 요소를 다른 배열에 push한다

// return문이 true일 때 해당하는 요소값이 짝수배열에 push된다
// ※※ return 결과가 짝수배열에 저장되는 것이 아님!!! ※※
